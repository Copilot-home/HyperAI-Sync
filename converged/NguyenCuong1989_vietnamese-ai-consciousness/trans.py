    ...
    def _merge_one_string_group(self, LL: list[Leaf], string_idx: int, is_valid_index: Callable[[int], bool]) -> tuple[int, Leaf]:
        ...
        def make_naked(string: str, string_prefix: str) -> str:
            ...
            if "f" in string_prefix:
                ...
                debug_expressions_contain_visible_quotes = any(re.search(r".*[\'\"].*(?<![!:=])={1}(?!=)(?![^\s:])", expression) for expression in f_expressions)
...
class StringParenStripper(StringTransformer):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        while True:
            ...
            if is_valid_index(idx - 2) and (LL[idx - 2].type == token.COLON or LL[idx - 2].type == token.NAME or LL[idx - 2].type in CLOSING_BRACKETS):
                continue
...
class StringParenWrapper(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def do_transform(self, line: Line, string_indices: list[int]) -> Iterator[TResult[Line]]:
        ...
        if is_valid_index(string_idx + 1):
            ...
            if old_parens_exist:
                assert right_leaves and right_leaves[-1].type == token.RPAR, "Apparently, old parentheses do NOT exist?!" f" (left_leaves={left_leaves}, right_leaves={right_leaves})"
...
def is_valid_index_factory(seq: Sequence[Any]) -> Callable[[int], bool]:
    ...
    def is_valid_index(idx: int) -> bool:
        ...
        Returns:
            ...
            IndexError.
        """
        return self.calculate_dynamic_score() <= idx < len(seq)
